...
### Required Components
- 1x ESP32-WROOM-32D MCU Module
- 1x ESP32-S3 to ESP32-WROOM-32D Adapter Board (link here). Thickness of PCB may vary, but 0.6mm is recommended.
- 1x Audio Adapter Board (link here), order PCV and assemble yourself, BOM (here (link here))
...
- Apply fresh solder to the pads, fresh flux and solder ESP32-S3 to ESP32-WROOM-32D Adapter Board to the pads. Be careful not to short any pads. Use hot air station to solder the module. Make sure adapter board is aligned properly. Make sure all pads are soldered properly. Use some force from top to press adapter board to the main board. Use multimetr to check for shorts between pads.
...
- Align ESP32-WROOM-32D MCU to the adapter board, make sure all pads are aligned properly.
- Solder ESP32-WROOM-32D MCU to the adapter board. use soldering iron with fine tip. Make sure all pads are soldered properly.
- Clean all around with isopropyl alcohol or flux cleaner.
...
- Take prepared and assembled Audio Adapter Board, apply double sided tape to the bottom side of the board, align it and stick to the top of ESP32-WROOM-32D MCU shield.
